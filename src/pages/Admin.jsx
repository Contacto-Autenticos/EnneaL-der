...
const Admin = () => {
    ...
    const [selectedCodeForUsages, setSelectedCodeForUsages] = useState(null);
    ...
    const fetchWorkshopRegistrations = async () => {
        ...
        try {
            // Fetch workshop registrations from the specialized table
    ...
    return (
        <div className="admin-shell">
                                    ...
                                    {loadingBusiness ? (
                                        ...
                                        businessResults.map((r) => (
                                            <tr key={r.id}>
                                                ...
                                                <td>{r.resp_name} {r.resp_lastname}</td>
                                                <td>{r.resp_role}</td>
                                                <td>{r.email}</td>
                                                <td>{r.phone}</td>
                        ...
                        <div className="codes-list-container">
                            ...
                            <div className="codes-table-wrapper" style={{ maxHeight: '400px' }}>
                                <table className="codes-table">
                                    ...
                                    <tbody>
                                        {codes.length === 0 ? (
                                            <tr><td colSpan="7" style={{ textAlign: 'center', padding: '10px' }}>
                                            ...
                                            codes.slice(0, 50).map((item) => {
                                                ...
                                                return (
                                                    <tr key={item.code}>
                                                        ...
                                                        <td style={{ fontSize: '0.8rem', color: '#111827', fontWeight: '500' }}>
                                                            {item.is_multi_use ? (
                                                                <button 
                                                                    onClick={() => setSelectedCodeForUsages(item)}
                                                                    className="btn-view-usages"
                                                                    style={{ 
                                                                        display: 'flex', 
                                                                        alignItems: 'center', 
                                                                        gap: '5px',
            ...
            {selectedCodeForUsages && (
                <div className="modal-overlay animate-fade-in" onClick={() => setSelectedCodeForUsages(null)}>
                    <div className="modal-content animate-slide-up" onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <h3><RefreshCw size={18} /> Historial de usos: {selectedCodeForUsages.code}</h3>
                            <button className="btn-close-modal" onClick={() => setSelectedCodeForUsages(null)}><X size={20} /></button>
                        </div>
                                            ...
                                            <tbody>
                                                {usages.map((u, idx) => (
                                                    <tr key={u.id || idx}>
                                                        <td>{u.user_email}</td>
                                                        <td>
                                                            <span className={`program-badge ${u.program.toLowerCase()}`}>
                                                                {u.program}
                                                            </span>
                                                        </td>
                        ...
                        <div className="modal-footer">
                            <button className="admin-btn-login" style={{ padding: '10px 20px', width: 'auto', margin: '0' }} onClick={() => setSelectedCodeForUsages(null)}>
                                Cerrar
                            </button>
                        </div>
                    </div>
